...
const Part2 = () => {
    ...
    function parseEntries(entries) {
        entries = entries.map(entry => {
            ...
            let newEntry = { time, state, raw: entry };
    ...
    function loadGuards(guards, entries) {
        Object.keys(guards).forEach(guardId => {
            ...
            guards[guardId].mostSleptMinute = Object.keys(sleepingMinutes).reduce(
                (prev, cur) => {
                    if (prev) {
    ...
    function getMostTiredGuardId(guards) {
        let guardId = Object.keys(guards).sort((prev, cur) => {
            let guard1 = guards[cur];
            let guard2 = guards[prev];

            // handle always awake guards >;(
            if (!guard1.mostSleptMinute) {
                return -1;
            } else if (!guard2.mostSleptMinute) {
                return 1;
            }

            if (guard1.sleepingMinutes[guard1.mostSleptMinute] > guard2.sleepingMinutes[guard2.mostSleptMinute]) {
                return 1;
            }
